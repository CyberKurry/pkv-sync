...
管理后台包含仪表盘、用户、响应式用户详情页、设备 token、笔记库、邀请码、运行时设置、活动记录和 blob 垃圾回收页面。

## 用户管理

- 在 **用户** 页面创建用户。
- 打开用户详情页可以重置密码、启用或禁用账号、提升或降低管理员权限，并管理该用户的设备 token。
...
- 设备 token 有效期为 90 天，可在全局 **设备** 页面或单个用户详情页撤销。
- 同一个稳定插件设备 ID 再次登录时，会替换该设备旧的活跃 token。
- 删除笔记库会同时删除元数据和服务端数据目录中的后端 Git 仓库。
