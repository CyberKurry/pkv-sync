...
使用 `pkvsyncd upgrade --version 1.0.3` 可以指定 release。若命令找不到匹配资产或校验和，请手动从 GitHub release 下载，并自行校验 `SHA256SUMS`。
...
- 运行 `pkvsyncd mcp --transport http --bind 127.0.0.1:6711` 通过 Streamable HTTP 暴露读写 MCP 服务给 AI 工具，或使用 `pkvsyncd mcp --vault <id>` 启动仅 stdio 的单笔记库会话。
- 大量删除附件后运行 blob 垃圾回收。
