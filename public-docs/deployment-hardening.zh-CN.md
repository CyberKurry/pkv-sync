...
1.2.1 补丁也收紧了暴露边界：Git HTTP Basic 失败信息保持泛化，MCP JSON 请求体上限为 100 MiB，blob 元数据检查会拒绝符号链接的 blob 路径，而不是跟随它们。
...
- 失败 bearer token 认证限流：按客户端 IP 分桶，每 60 秒最多 120 次失败尝试。
- 最大文件大小，默认 `100 MiB`。
- 支持的文本扩展名。
- 时区，默认 `Asia/Shanghai`。

注册、登录失败和失败 bearer token 认证都会被限流。Setup、公开注册、用户自助修改密码，以及管理员创建或重置的密码都必须至少 12 个字符，并包含大写字母、小写字母和数字；CLI 创建的用户也仍应使用强密码。

Blob 上传请求体受 `max_file_size` 限制，并且始终会被硬 blob 上限夹住（生产环境 `512 MiB`）。主 SSE 流在保持打开时会复查 bearer token；MCP 读取和搜索工具也有响应大小与总搜索预算，避免大型笔记库被展开成无界 JSON 响应。

Pull/tree 遍历和 rollback 可达性检查都有边界；被当前同步过滤规则拒绝的路径会从读取、历史、diff 和 commit-list 界面隐藏。
...
内置 backup/restore helper 不会跟随 symlink。`vaults/` 或 `blobs/` 下的 symlink 条目会在备份时跳过，在恢复清理时只移除链接本身，不会触碰链接目标。
...
设备 bearer token 会在认证请求时续期，连续 90 天未使用才会过期，单个 token 最长有效 365 天，也可以由用户或管理员撤销。在过期或撤销前，请把活跃 token 当作凭据处理。

Obsidian 会把插件的活跃 token、部署密钥、登录状态和稳定设备身份保存在设备本地存储中。笔记库本地插件 `data.json` 只保留非敏感偏好和同步索引；当前版本的同步索引 key 不再包含部署密钥，旧版带敏感信息的索引项会在下次写入插件数据时被丢弃。请提醒用户保护 Obsidian 设备本地存储、共享压缩包、不可信同步目标、明文备份以及旧版本留下的 `data.json` 副本。如果这些存储可能泄露，请撤销受影响的设备 token；如果部署密钥曾经暴露，请轮换部署密钥。
