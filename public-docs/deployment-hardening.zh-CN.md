...
5. 备份当前数据。
6. 二进制部署先运行 `pkvsyncd upgrade --dry-run` 预览 release 资产，再运行 `pkvsyncd upgrade --yes` 下载并校验当前二进制旁边的 `pkvsyncd.new`。只有校验通过后，才停止服务并替换二进制。
7. Docker 或 Kubernetes 部署应拉取或修改镜像 tag 并重启服务或 rollout，不要在容器内替换二进制。
8. 用新二进制或新镜像运行 migrations。
