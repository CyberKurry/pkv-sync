...
`pkvsyncd` 提供十个子命令。最常用的运维流程是 `serve`、`genkey`、`migrate up`、`user add`、`backup` 和 `restore`。
...
- 用 `git2` 打开裸仓库并遍历，报告仓库与对象级别的读取错误。
...
- `--yes`：跳过交互式确认提示。
- `--version <VERSION>`：下载指定版本（例如 `1.5.1`），而不是最新发行版。
