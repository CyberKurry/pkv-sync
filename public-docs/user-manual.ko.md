...
Version 1.2.1은 sync 중 중복 plugin 작업을 줄입니다. pull과 push 단계는 가능한 경우 같은 scan을 재사용하고, conditional pull은 추가 request를 피하며, 변경 파일 snapshot은 제한된 병렬 처리로 처리됩니다.
...
`.conflict-*` 파일이 생성되면 이후 동기화에서 제외됩니다. Obsidian에서 두 파일을 검토하고 유지할 콘텐츠를 병합한 다음 충돌 파일을 삭제하십시오.
