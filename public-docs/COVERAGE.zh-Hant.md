...
Rust 覆蓋率只在 Ubuntu CI runner 上透過 `cargo tarpaulin` 產生。不要將 Windows 本機 tarpaulin 輸出提升為此基線。門禁允許每個 component 最多下降 5.0 個百分點。
...
v0.5.1 基線來自 CI run `26225831124`。Rust server 覆蓋率採用 Ubuntu tarpaulin artifact（`90.95%`）。Obsidian 外掛覆蓋率採用同一次 run 的 Vitest/V8 summary（`48.42%`）；這次 run 包含卸載回歸測試觸達的外掛 entrypoint 與 UI import graph，因此量測範圍比前一個外掛基線更廣。
