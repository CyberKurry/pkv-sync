...
`pkvsyncd` は 10 のサブコマンドを提供します。最も一般的な運用フローは `serve`、`genkey`、`migrate up`、`user add`、`backup`、`restore` です。
...
- `git2` で bare リポジトリを開いて走査し、リポジトリおよびオブジェクト単位の読み取りエラーを報告します。
...
- `--version <VERSION>`: 最新リリースではなく `1.5.1` のような特定のリリースをダウンロードします。
