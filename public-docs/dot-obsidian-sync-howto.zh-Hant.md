...
- **Obsidian 外掛：設定 -> PKV Sync -> Apply recommended starter list** 只寫入兩條 glob（`.obsidian/themes/**` 與 `.obsidian/snippets/**`）—— 主題與 CSS snippets 通常可以安全跨裝置共享。
...
- Obsidian：開啟 **設定 -> PKV Sync**，選擇目前 vault，編輯 **.obsidian 同步允許清單**，然後點選 **儲存**。
