...
v0.5.1 基线来自 CI run `26225831124`。Rust 服务端覆盖率采用 Ubuntu tarpaulin artifact（`90.95%`）。Obsidian 插件覆盖率采用同一次运行的 Vitest/V8 汇总（`48.42%`）；这次运行包含卸载回归测试触达的插件入口点和 UI import graph，因此统计范围比上一个插件基线更广。
