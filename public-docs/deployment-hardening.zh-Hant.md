...
將 `[server].public_host` 設定為運維實際存取 admin 面板使用的外部主機名稱（必要時含連接埠），例如 `sync.example.com` 或 `pkv.local:8443`。admin CSRF 檢查依據該值計算期望 Origin。
...
倉庫提供了 `deploy/nginx/pkv-sync.conf`。它會把 HTTP 跳轉到 HTTPS，設定 `client_max_body_size 110m`，並轉發 PKV Sync 用於 Host 和用戶端 IP 處理的 header。
...
5. 備份目前資料。
6. 用新二進位執行 migrations。

Migration 一旦發布就應保持追加式管理。不要為了既有部署壓縮已發布 migration。
