...
PKV Sync 1.2.0 は、LLM が保守する Wiki ワークフロー向けの現在の安定リリースです。原子的な `write_files` 複数ファイル MCP コミットと、履歴を保つ `move_file` リネーム／移動ツールを追加し、v1.1.1 のセキュリティ強化も含んでいます。Agent ループは [LLM Wiki ハウツー](./public-docs/llm-wiki-howto.ja.md) を参照してください。
