...
PKV Sync 1.3.0 はプッシュファースト同期プロトコルによる本格的な自動マージを導入しています。同一ファイルの異なる行への並行編集は、両デバイスで自動的にマージされ、コンフリクトファイルは生成されません。クライアントは未確認のマージコミットを越えて head を進めることがなくなり、サイレントデータ損失のバグが解消されました。同期内部の実装は保守性向上のためモジュール化されました。
