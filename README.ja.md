...
PKV Sync 1.4.4 は包括的な監査主導のクリーンアップを完了：重複ヘルパー関数の統合（blob パス分割、vault-ID 検証、blob-pointer 解析）、8 つの未使用ランタイム設定 setter の削除、push・pull・scan・MCP ツールにおける 15 項目のパフォーマンス改善——正規表現コンパイルの巻き上げ、空操作の短絡、ツリー読み取りのバッチ化、競合カウントのキャッシュ。
