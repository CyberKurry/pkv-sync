...
PKV Sync 1.0.14 は現在の安定 patch リリースです。この版では、reverse proxy 配下のスタンドアロン MCP HTTP transport が実際の client IP で正しく rate-limit され、キャンセルされた認証試行は IP ごとの失敗バジェットを消費しなくなり、auto-merge 競合 sidecar は exclude ルールに一致しても保持され、MCP の書き込みは設定された最大ファイルサイズまで許可されます。
