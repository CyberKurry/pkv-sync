...
| LLM が保守する Wiki ワークフロー | [LLM Wiki ハウツー](./public-docs/llm-wiki-howto.ja.md) |
...
PKV Sync 1.2.1 は、LLM が保守する Wiki ワークフロー向けの現在の安定パッチリリースです。MCP と Git の認証、隠しパス、重複書き込み、blob シンボリックリンクの境界を強化し、大規模 vault の push/pull、MCP ツール経路、クリーンアップ、プラグインスキャンをより増分的かつキャッシュ効率の高いものにしました。Agent ループは [LLM Wiki ハウツー](./public-docs/llm-wiki-howto.ja.md) を参照してください。

このリリースの要点:

- MCP writes は正規化後の重複パスを拒否し、hidden/excluded target の存在を漏らさず、無効な move source では write quota を消費しません。
- 大規模 vault の sync path は、増分 Git tree mutation、増分 push metadata 更新、vault filter と token check の cache、batch pull reads、より高速な plugin scan を使います。
- Admin と運用画面は実際の user vault stats、ローカライズ済み duration labels、server/plugin/OpenAPI/public docs metadata の整合を反映します。
