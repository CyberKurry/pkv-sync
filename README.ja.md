...
PKV Sync 1.4.1 は、深度セキュリティ監査を経て展開と信頼性を包括的に強化しました。コンテナイメージは実行時ヘルスチェック・固定イメージタグ・Caddy セキュリティヘッダーとリクエストボディ上限を備え、非同期 diff／履歴／競合モーダルは閉じた後に分離済み DOM への書き込みを行わず、競合解決後に即座に push し、復元確認テキストはファイル名の置換構文（$&／$’ など）でも壊れません。
