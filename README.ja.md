...
PKV Sync 1.1.0 は現在の安定機能リリースです。LLM が管理する wiki ワークフロー向けに MCP の `link_graph` と `changes_since` 読み取りツールを追加し、新しい LLM Wiki ガイドを公開し、SyncPathFilter で拒否されたパスがファイル一覧、読み取り、検索、リンクグラフ、変更サマリーに表示されないよう MCP のすべての読み取り面を強化しました。
