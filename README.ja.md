...
ありません。プラグインの非機密設定と同期インデックスは
`<vault>/.obsidian/plugins/pkv-sync/data.json` に保存されます。ログイン状態、
現在の bearer デバイストークン、デプロイメントキー、安定したデバイス ID は
Obsidian のデバイスローカルストレージに保存されます。Obsidian の
デバイスローカルストレージ、平文バックアップ、旧バージョンから残ったプラグイン
`data.json` のコピーは機密として扱ってください。デバイストークンは使用時に更新され、
90 日アイドルで失効し、各トークンには 365 日の絶対有効期限があります。同じデバイスで再ログインすると有効なトークンが入れ替わります。
...
PKV Sync 1.4.3 は監査修正の続きで、正確性に重点を置いています。監視付きバックグラウンドタスクはグレースフルシャットダウン時に中止され、古い DashMap エントリは定期的に回収され、自動マージは欠落オブジェクトと Git 一時エラーを正しく区別し、冪等キャッシュはメタデータトランザクション失敗後に必ず書き込まれ、並行テキスト作成はコンフリクトファイル昇格で保持されます。不要コード（未使用ヘルパー、i18n キー、Docker レイヤー）も整理されました。
