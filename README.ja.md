...
PKV Sync 1.2.3 は現在の安定パッチリリースです。コミュニティディレクトリのソース lint の指摘を解消しました。プラットフォーム検出は Platform API を使用するようになり、フォルダー検索は宣言された最小アプリバージョンの範囲内にとどまり、コマンド ID から冗長なプラグインプレフィックスを削除しました。
