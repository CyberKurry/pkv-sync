...
PKV Sync 1.4.0 は、異なるデバイスから同じ行に加えられた重ならない編集を文字単位で自動マージし、さらに systemd と Docker 向けにオプトインの「通知 + ワンクリック」セルフアップグレード（ヘルスチェック失敗時の自動ロールバック付き）を追加しました。サーバー自体は非特権のままです。
