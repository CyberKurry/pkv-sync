...
PKV Sync 1.0.13 は現在の安定 patch リリースです。この版には最新の security/performance 実装が含まれます。登録とパスワード変更には setup と同じ強度ポリシーを適用し、除外されたパスは read/history/diff API から隠され、rollback と pull の作業量には上限があり、backup/restore helper は symlink をたどらず、blob 参照メタデータ修復も高速化されています。
