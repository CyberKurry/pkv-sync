...
PKV Sync 1.4.2 はセキュリティ監査後のパッチです。プラグイン認証情報は同期されるプラグインデータから除去され、利用可能な環境では Electron safeStorage で暗号化保存されます。古い秘密入り同期インデックスは破棄され、Git smart HTTP は main ref のみを公開して到達不能オブジェクトを prune し、サーバー側の vault path と blob hash 検証は共有境界で fail closed します。
