...
本地文件就是真相——插件直接读写你磁盘上的 Obsidian 笔记库，不存在代理文件系统这种东西。插件设置和当前的设备令牌都存在 `<vault>/.obsidian/plugins/pkv-sync/data.json`，请把这个文件当成敏感数据。设备令牌在使用时会自动续期，90 天无活动后失效，且单个令牌最长有效 365 天；在同一设备重新登录会轮换掉旧令牌。
...
PKV Sync 1.1.0 是当前稳定功能版本。本版新增用于 LLM 维护 wiki 工作流的 MCP `link_graph` 和 `changes_since` 读取工具，发布新的 LLM Wiki 指南，并强化所有 MCP 读取表面：被 SyncPathFilter 拒绝的路径会继续隐藏于文件列表、读取、搜索、链接图和变更摘要之外。
