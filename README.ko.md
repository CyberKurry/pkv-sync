...
PKV Sync 1.1.0은 현재 안정 기능 릴리스입니다. LLM이 관리하는 wiki 워크플로를 위해 MCP `link_graph` 및 `changes_since` 읽기 도구를 추가하고, 새 LLM Wiki 가이드를 문서화했으며, SyncPathFilter가 거부한 경로가 파일 목록, 읽기, 검색, 링크 그래프, 변경 요약에 표시되지 않도록 모든 MCP 읽기 표면을 강화했습니다.
