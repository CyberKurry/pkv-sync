...
- **어드민 패널**은 5 개 언어(English, 简中, 繁中, 日本語, 한국어)로
  사용자, 기기 토큰, 볼트, 초대, 활동, 블롭 GC 를 관리하며 파괴적인 볼트와
  사용자 작업에는 확인 대화상자를 표시합니다.
- **AI가 읽을 수 있는 vault.** MCP는 stdio, 독립 Streamable HTTP, 또는 `pkvsyncd serve`에 내장된 `/mcp` 라우트로 읽기/쓰기 도구를 노출합니다.
- **기본적으로 경계가 있습니다.** 관리자가 생성／재설정하는 비밀번호는 setup과 같은 강한 정책을 사용하고, token 평문은 한 번만 표시되며, upload와 MCP response에는 크기 상한이 있고, live SSE stream은 철회된 token을 재검증합니다.
...
2. `docker-compose.yml` 옆에 `config.toml` 을 둡니다.
   ...
   [mcp]
   embed_in_serve = false                # true이면 이 서버에 /mcp를 마운트
   ```
...
PKV Sync는 MCP Streamable HTTP transport를 두 가지 방식으로 노출합니다. 내장
모드는 명시적으로 켭니다. `[mcp].embed_in_serve = true`를 설정하면 `pkvsyncd
serve`가 메인 서버 포트에 `/mcp`를 마운트하고 같은 TLS 종료, 리버스 프록시,
배포 키, bearer token 검증을 공유합니다. 독립 모드는 기존처럼 별도 프로세스입니다:
`pkvsyncd mcp --transport http --bind 127.0.0.1:6711`. MCP 격리, 전용 bind
address, 또는 독립 scaling이 필요할 때 유용합니다.

## Obsidian 플러그인

로컬 파일이 source of truth 입니다. 플러그인은 디스크 위의 평범한
Obsidian 볼트를 읽고 쓰며, 프록시 파일시스템을 만들지 않습니다. 플러그인
민감하지 않은 설정과 동기화 인덱스는
`<vault>/.obsidian/plugins/pkv-sync/data.json` 에 저장됩니다. 로그인 상태,
활성 bearer 기기 토큰, 배포 키, 안정적인 기기 ID는 Obsidian의 기기 로컬
저장소에 저장됩니다. Obsidian 기기 로컬 저장소, 평문 백업, 이전 버전에서 남은
플러그인 `data.json` 사본은 민감 정보로 취급하세요. 기기 토큰은 사용할 때마다
갱신되고 90 일 동안 비활성 상태이면 만료되며, 각 토큰에는 365 일의 절대 수명이
있습니다. 같은 기기에서 다시 로그인하면 활성 토큰이 교체됩니다.
...
| LLM이 유지관리하는 Wiki workflow | [LLM Wiki 사용법](./public-docs/llm-wiki-howto.ko.md) |
...
PKV Sync 1.4.2는 보안 감사 이후의 패치입니다. 플러그인 자격 증명은 동기화되는 플러그인 데이터에서 제거되고, 가능한 환경에서는 Electron safeStorage로 암호화되어 저장됩니다. 오래된 비밀 포함 동기화 인덱스는 폐기되며, Git smart HTTP는 main ref만 노출하고 도달 불가능한 객체를 prune하고, 서버의 vault path 및 blob hash 검증은 공통 경계에서 fail closed 합니다.
