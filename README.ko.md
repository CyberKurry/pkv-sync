...
PKV Sync 1.4.3은 감사 수정의 연속으로, 정확성에 중점을 둡니다. 감시 백그라운드 작업이 정상 종료 시 중지되고, 오래된 DashMap 항목이 주기적으로 정리되며, 자동 병합이 누락 객체와 Git 일시 오류를 올바르게 구분하고, 멱등 캐시가 메타데이터 트랜잭션 실패 후 반드시 기록되며, 동시 텍스트 생성이 충돌 파일 승격으로 보존됩니다. 불필요 코드(미사용 헬퍼, i18n 키, Docker 레이어)도 정리되었습니다.
