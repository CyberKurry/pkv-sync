...
PKV Sync 1.4.4는 포괄적인 감사 주도 정리를 완료합니다: 중복 헬퍼 함수 통합(blob 경로 샤딩, vault-ID 검증, blob-pointer 파싱), 8개의 미사용 런타임 구성 setter 제거, push·pull·scan·MCP 도구에서 15가지 성능 개선 제공 — 정규식 컴파일 호이스팅, 빈 작업 단락, 트리 읽기 배치, 충돌 카운트 캐싱.
