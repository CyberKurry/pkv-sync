...
PKV Sync 1.2.0은 LLM이 유지관리하는 Wiki workflow를 위한 현재 안정 릴리스입니다. 원자적 `write_files` 다중 파일 MCP 커밋과 이력을 보존하는 `move_file` 이름 변경／이동 도구를 추가했으며, v1.1.1 보안 강화도 포함합니다. Agent loop는 [LLM Wiki 사용법](./public-docs/llm-wiki-howto.ko.md)을 참고하세요.
