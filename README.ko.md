...
PKV Sync 1.1.1은 현재 안정 보안 패치 릴리스입니다. 자동 병합 충돌 사이드카가 사용자 제외 규칙과 일치해도 읽기 및 MCP 표면에서 계속 보이도록 하고, MCP 인증 오류 열거와 레이트 리밋 차감 경계 사례를 수정했으며, 신뢰 프록시 IP, 비밀번호 변경, 초대 등록, SQLite 권한, Git OID, 경로 처리, 요청 크기 제한을 강화했습니다.
