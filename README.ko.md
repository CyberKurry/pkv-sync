...
만료되며, 각 토큰에는 365 일의 절대 수명이 있습니다. 같은 기기에서 다시 로그인하면 활성 토큰이 교체됩니다.
...
PKV Sync 1.3.0은 푸시 우선(push-first) 동기화 프로토콜을 통해 진정한 자동 병합을 도입합니다. 같은 파일의 다른 줄에 대한 병행 편집은 양쪽 기기에서 자동으로 병합되며, 충돌 파일이 전혀 생성되지 않습니다. 클라이언트는 이제 확인되지 않은 병합 커밋을 넘어 head를 진행시키지 않으므로, 조용한 데이터 손실 버그가 해소되었습니다. 동기화 내부 구현은 유지보수성 향상을 위해 모듈화되었습니다.
