...
PKV Sync 1.4.1은 심층 보안 감사를 거쳐 배포와 신뢰성을 전반적으로 강화했습니다. 컨테이너 이미지는 런타임 상태 확인·고정 이미지 태그·Caddy 보안 헤더와 요청 본문 한도를 갖추고 있으며, 비동기 diff／히스토리／충돌 모달은 닫힌 후 분리된 DOM에 쓰지 않고, 충돌 해결 후 즉시 push하며, 복원 확인 텍스트는 파일명의 치환 문법($&／$’ 등)으로도 손상되지 않습니다.
