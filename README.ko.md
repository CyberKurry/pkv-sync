...
PKV Sync 1.4.0은 서로 다른 기기에서 같은 줄에 가한 겹치지 않는 편집을 문자 단위로 자동 병합하고, systemd 및 Docker 배포를 위한 옵트인 '알림 + 원클릭' 셀프 업그레이드(헬스 체크 실패 시 자동 롤백 포함)를 추가했습니다. 서버 자체는 비특권 상태를 유지합니다.
