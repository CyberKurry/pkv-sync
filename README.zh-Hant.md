...
PKV Sync 1.3.0 透過推優先（push-first）同步協定引入了真正的自動合併：對同一檔案不同行的並行編輯現在會在兩台裝置上自動合併，不產生任何衝突檔案。用戶端現在不會將自身的 head 推進到尚未見過的合併提交之後，從而消除了靜默資料遺失的缺陷。同步內部邏輯已模組化重構，以提升可維護性。
