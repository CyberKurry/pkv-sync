...
pub fn spawn_update_check(state: AppState, cfg: UpdateCheckConfig) -> tokio::task::JoinHandle<()> {
    ...
    crate::service::background::spawn_supervised(
        ...
        move || {
            ...
            async move {
                ...
                loop {
                    let sleep_for = match runtime_update_check_schedule(
                        &state.runtime_cfg.snapshot().await,
                    ) {
...
mod tests {
    ...
    #[test]
    fn update_check_task_is_supervised() {
        let source = include_str!("update_check.rs");
        let fn_start = source.find("pub fn spawn_update_check").unwrap();
        let next_fn = source[fn_start + 1..]
            .find("\nfn runtime_update_check_schedule")
